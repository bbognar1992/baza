"""
Task model for ÉpítAI Construction Management System
"""
...
class Task(Base, TimestampMixin):
    ...
    task_id = db(Integer, primary_key=True, autoincrement=True)
    phase_id = db(Integer, ForeignKey('phases.phase_id', ondelete='CASCADE'), nullable=False)
    name = db(String(300), nullable=False)
    description = db(Text)
    profession_type_id = db(Integer, ForeignKey('profession_types.profession_type_id'))
    duration_days = db(Integer, default=1)
    required_people = db(Integer, default=1)
    order_sequence = db(Integer, nullable=False)
