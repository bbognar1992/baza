...
class Material(Base, TimestampMixin):
    ...
    material_id = db(Integer, primary_key=True, autoincrement=True)
    resource_id = db(Integer, ForeignKey('resources.resource_id'))
    name = db(String(300), nullable=False)
    category = db(String(100))
    unit = db(String(50))
    unit_cost = db(Numeric(10, 2))
    description = db(Text)
    supplier = db(String(200))
    vendor_contact = db(String(255))
    lead_time_days = db(Integer, default=0)
    minimum_order = db(Integer, default=1)
    current_stock = db(Integer, default=0)
    reorder_level = db(Integer, default=0)
    status = db(String(50), default='Available')
...
class ProjectMaterial(Base, TimestampMixin):
    ...
    project_material_id = db(Integer, primary_key=True, autoincrement=True)
    project_id = db(Integer, ForeignKey('projects.project_id', ondelete='CASCADE'), nullable=False)
    material_id = db(Integer, ForeignKey('materials.material_id'), nullable=False)
    quantity = db(Numeric(10, 2), nullable=False)
    unit_cost = db(Numeric(10, 2))
    total_cost = db(Numeric(12, 2))  # Generated column: quantity * unit_cost
    assigned_date = db(Date, default='CURRENT_DATE')
    status = db(String(50), default='Planned')
