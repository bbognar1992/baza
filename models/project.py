"""
Project models for ÉpítAI Construction Management System
"""
...
class Project(Base, TimestampMixin):
    ...
    project_id = db(Integer, primary_key=True, autoincrement=True)
    project_name = db(String(300), nullable=False)
    client_name = db(String(200))
    project_type_id = db(Integer, ForeignKey('project_types.project_type_id'))
    status = db(String(50), nullable=False)
    start_date = db(Date, nullable=False)
    end_date = db(Date, nullable=False)
    budget = db(Numeric(15, 2))
    project_manager_id = db(Integer, ForeignKey('users.user_id'))
    location = db(String(500))
    description = db(Text)
    priority = db(String(20))
    progress_percent = db(Integer, default=0)
    size_sqm = db(Integer)
    project_code = db(String(100), unique=True)
...
class ProjectLocation(Base, TimestampMixin):
    ...
    project_location_id = db(Integer, primary_key=True, autoincrement=True)
    project_id = db(Integer, ForeignKey('projects.project_id', ondelete='CASCADE'), nullable=False)
    location_name = db(String(200), nullable=False)
    address = db(Text)
    coordinates_lat = db(Numeric(10, 8))
    coordinates_lng = db(Numeric(11, 8))
...
class ProjectMember(Base, TimestampMixin):
    ...
    project_member_id = db(Integer, primary_key=True, autoincrement=True)
    project_id = db(Integer, ForeignKey('projects.project_id', ondelete='CASCADE'), nullable=False)
    resource_id = db(Integer, ForeignKey('resources.resource_id', ondelete='CASCADE'), nullable=False)
    role_in_project = db(String(100))
    assigned_date = db(Date, default='CURRENT_DATE')
