...
class Resource(Base, TimestampMixin):
    ...
    resource_id = db(Integer, primary_key=True, autoincrement=True)
    type = db(String(50), nullable=False)
    name = db(String(200), nullable=False)
    position = db(String(100))
    profession_type_id = db(Integer, ForeignKey('profession_types.profession_type_id'))
    phone = db(String(20))
    email = db(String(255))
    address = db(Text)
    skills = db(Text)
    hourly_rate = db(Numeric(10, 2), default=0)
    availability = db(String(50), default='Elérhető')
    experience_years = db(Integer, default=0)
